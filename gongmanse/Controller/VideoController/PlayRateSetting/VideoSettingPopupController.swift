...
class VideoSettingPopupController: BottomPopupViewController {
    ...
    func configureUI() {
        ...
        tableView.register(VideoSettingPopupCell.self, forCellReuseIdentifier: VideoSettingPopupCell.reusableIdentifier)
...
extension VideoSettingPopupController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: VideoSettingPopupCell.reusableIdentifier,
                                                 for: indexPath) as! VideoSettingPopupCell
        cell.imageName = indexPath.row == 0 ? "textbox" : "speedometer"
        cell.cellLabel.text = indexPath.row == 0 ? "자막 표시" : "재생 속도"
        cell.state.text = indexPath.row == 0 ? "있음" : "기본"
        return cell
    }
