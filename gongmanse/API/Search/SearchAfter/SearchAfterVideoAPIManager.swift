...
struct SearchAfterVideoAPIManager {
    ...
    func fetchVideoAPI(_ parameters: SearchVideoPostModel,
        ...
        AF.upload(multipartFormData: { formData in
            ...
            formData.append("\(data.offset ?? "")".data(using: .utf8) ?? Data(), withName: "offset")
            formData.append("\(data.sortid ?? "")".data(using: .utf8) ?? Data(), withName: "sortid")
            formData.append("\(data.limit ?? "")".data(using: .utf8) ?? Data(), withName: "limit")
