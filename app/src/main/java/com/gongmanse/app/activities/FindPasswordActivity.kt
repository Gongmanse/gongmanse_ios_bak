...
import com.gongmanse.app.databinding.ActivityFindPasswordBinding
...
class FindPasswordActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun initView() {
        ...
        binding.layoutToolbar.title = Constants.ACTIONBAR_TITLE_FIND_PASSWORD
...
//                }
    ...
    private fun setRequestView(isChecked: Boolean) {
        if (!isChecked) {
            binding.btnRequestAuth.apply {
                isClickable = false
                mTimerRunning = false
                setCardBackgroundColor(ContextCompat.getColorStateList(this@FindPasswordActivity, R.color.gray))
            }
        } else {
            binding.btnRequestAuth.apply {
                isClickable = true
                mTimerRunning = false
                setCardBackgroundColor(ContextCompat.getColorStateList(this@FindPasswordActivity, R.color.main_color))
            }
        }
    }
    ...
    private fun checkAuthNumber() {
        ...
        startActivity(intentFor<EnterNewPasswordActivity>(Constants.EXTRA_KEY_USER_NAME to userId).singleTop())
        finish()
    }
    ...
    private fun requestAuthNumber() {
        ...
        binding.apply {
            ...
            tvCounter.setTextColor(ContextCompat.getColor(this@FindPasswordActivity, R.color.sub_color7))
            ...
            Log.d(TAG, "receiverType => $receiverType")
            Log.d(TAG, "userReceiver => $userReceiver")
            Log.d(TAG, "userName => $userName")
            RetrofitClient.getService().requestAuthNumber(receiverType, userReceiver, userName).enqueue(object :
    ...
    private fun resetView(checkedId: Int) {
        ...
        binding.apply {
            ...
            btnRequestAuth.setCardBackgroundColor(ContextCompat.getColor(this@FindPasswordActivity, R.color.gray))
            tvCounter.setTextColor(ContextCompat.getColor(this@FindPasswordActivity, R.color.gray))
            tvCounter.text = getString(R.string.content_text_counter)
    ...
    private fun setMaxLength(length: Int?) {
        binding.inputAuth.filters = if (length == null)
            Array<InputFilter>(1) { InputFilter.LengthFilter(Int.MAX_VALUE) }
        else
            Array<InputFilter>(1) { InputFilter.LengthFilter(length) }
    }
            ...
            override fun onResponse(call: Call<Data>, response: Response<Data>) {
                if (response.isSuccessful) {
                    response.body()?.let {
                        ...
                        when (it.data) {
                            CODE_00 -> Toast.makeText(this@FindPasswordActivity, "존재하지 않는 사용자입니다.", Toast.LENGTH_SHORT).show()
                            CODE_10 -> Toast.makeText(this@FindPasswordActivity, "아이디가 일치하지 않습니다.", Toast.LENGTH_SHORT).show()
                            CODE_11 -> {
                                if (receiverType == Constants.RECEIVER_TYPE_EMAIL) {
